...
export const createGifted = async (body: Gifted) => {
  ...
  const existing = (
    await prisma.gifted.findUnique({ where: { key: body.key }, select: { email: true } })
  )?.email
...
export const updateGiftedData = async (
  ...
  if (giftedData.status === Status.APPROVED)
    return { status: 400, message: 'Gifted was already approved' }
  try {
