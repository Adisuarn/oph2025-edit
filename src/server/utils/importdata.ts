...
export async function importClubData() {
  ...
  for (const item of data) {
    const key = Object.keys(AllData.Clubs).find((k) => AllData.Clubs[k as keyof typeof AllData.Clubs] === item.name);
...
export async function importProgramData() {
  ...
  for (const item of data) {
    const key = Object.keys(AllData.Programs).find((k) => AllData.Programs[k as keyof typeof AllData.Programs] === item.name);
    console.log(`Importing ${item.name}, existing key: ${key}`);
    const transformedItem = {
      thainame: item.name,
      name: item.engname || "",
...
export async function importGiftedData() {
  ...
  for (const item of data) {
    const key = Object.keys(AllData.Gifted).find((k) => AllData.Gifted[k as keyof typeof AllData.Gifted] === item.name);
    console.log(`Importing ${item.name}, existing key: ${key}`);
    const transformedItem = {
      thainame: item.name,
      name: item.engname || "",
...
export async function importOrganizationData() {
  ...
  try {
    for (const item of data) {
      const key = Object.keys(AllData.Organizations).find((k) => AllData.Organizations[k as keyof typeof AllData.Organizations] === item.name);
      console.log(`Importing ${item.name}, existing key: ${key}`);
      const transformedItem = {
        thainame: item.name,
        name: item.engname || "",
