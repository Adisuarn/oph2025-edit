...
const Forms: React.FC<FormProps> = ({ dataRecord }) => {
  ...
  const onSubmit = useCallback(async (values: { tagOptions: string; keyOptions: string }) => {
    ...
    <main className="flex h-screen w-screen items-center justify-center relative overflow-hidden bg-gradient-to-b from-[#ECF5C8] to-[#1A8B6D]">
      ...
      >
        {({ values, setFieldValue }) => {
          ...
          const isKeyOptionsDisabled = !values.tagOptions; // Disable if no tag option is selected
          ...
          return (
            <Form>
              <div className="-mt-14 flex h-[60vh] w-[80vw] sm:w-[50vw] flex-col items-center justify-center space-y-5 rounded-2xl bg-white relative z-50 shadow-xl">
                ...
                <div className="relative sm:w-3/5 w-4/5 rounded-md border border-gray py-2 pl-2 pr-12">
                  <span className="absolute -top-2 left-3 bg-white px-1 text-[#A9A9A9] text-xs">Email</span>
                  <p className="text-[#A9A9A9] text-xs sm:text-md md:text-lg">{dataRecord.email}</p>
                </div>
                    ...
                    <div key={value} className="space-x-2">
                      <Field
                        ...
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                          ...
                          setFieldValue("keyOptions", ""); // Reset keyOptions
                        }}
                      />
                ...
                <Field
                  as="select"
                  name="keyOptions"
                  className={`form-select border border-gray pl-2 py-2 px-8 rounded-md bg-white ${values.keyOptions ? "text-black" : "text-[#A9A9A9]"}`}
                  disabled={isKeyOptionsDisabled} // Disable if no tag option is selected
                >
                ...
                <button
                  className={`text-indigo-500 px-4 py-2 text-center border border-indigo-500 rounded-lg 
